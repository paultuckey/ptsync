...
//! [`crate::media::best_guess_title`] and [`crate::media::best_guess_description`],
...
use crate::exif_util::exif_datetime_to_rfc3339;
...
pub(crate) struct PsXmpInfo {
    ...
    /// (see [`crate::media::best_guess_taken_dt`], which parses it back).
...
///
/// Two conventions are in the wild and both are tried, most specific first:
///
/// 1. **Appended** — `IMG_1234.JPG.xmp`. darktable, digiKam and ExifTool's usual
///    recipe. Unambiguous: the media extension is part of the name.
/// 2. **Swapped** — `IMG_1234.xmp`. Adobe's convention, inherited from raw
///    workflows where one negative had one sidecar. Ambiguous as soon as
///    `IMG_1234.JPG` and `IMG_1234.HEIC` share a directory — both claim the same
///    sidecar — which is exactly why form 1 is preferred when both are present.
///
...
/// Sidecar names to try for `path`, most specific first. Split out from
/// [`detect_xmp`] so the ordering can be asserted without a filesystem.
fn xmp_candidates(path: &str) -> Vec<String> {
    let mut candidates = vec![format!("{path}.xmp"), format!("{path}.XMP")];
    // The swapped form only differs when there is an extension to swap; a
    // dot-less name would just re-derive the appended candidates above.
...
pub(crate) fn parse_xmp(text: &str) -> Option<PsXmpInfo> {
    ...
    // [`crate::exif_util::parse_exif_info`] and
    // [`crate::supplemental_info::parse_supplemental_info`].
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
 <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  <rdf:Description xmlns:exif="http://ns.adobe.com/exif/1.0/"
    ...
    #[test]
    fn test_candidate_order_prefers_appended_form() {
        let candidates = xmp_candidates("a/IMG_1234.JPG");
        assert_eq!(candidates[0], "a/IMG_1234.JPG.xmp");
        assert_eq!(candidates[1], "a/IMG_1234.JPG.XMP");
        assert_eq!(candidates[2], "a/IMG_1234.xmp");
        assert_eq!(candidates[3], "a/IMG_1234.XMP");
        // No extension: only the appended form is meaningful.
