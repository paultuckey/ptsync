...
`docs/demo.gif` (shown at the top of the README) is generated by the
`tests/demo.rs` integration test, never recorded by hand. Regenerate it after
changing anything the demo shows — console output, the output directory layout,
or the markdown frontmatter:

```shell
UPDATE_DOCS=1 cargo test --test demo
```
