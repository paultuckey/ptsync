...
Supported formats: images (JPG, PNG, HEIC, GIF) and video (MP4, M4V, MOV, AVI, MPG, WMV).
Other file types are skipped. AVI, MPG and WMV carry no embedded capture time, so those
files are dated from their sidecar metadata or file timestamps.
...
- **Timezones.** The archive is laid out on the photographer's wall clock: a photo taken
  ...
  nothing in those files says where the shutter was. To pin a particular one instead, set the env var 
  `PTSYNC_TZ` to a UTC offset - `+12:00`, `-04:00`, `+0545` or `UTC`.
